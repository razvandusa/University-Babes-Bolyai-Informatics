...
void test_write_to_file() {
    std::ofstream fout(file_name + "/tests/tests_file.txt", std::ofstream::trunc);
    fout.close();

    RepositoryFile repository(file_name + "/tests/tests_file.txt");
    repository.add_disciplina(Disciplina(1,"Limba Romana",4,"Obligatorie","Andrei Maruta"));

    std::ifstream fin(file_name + "/tests/tests_file.txt");
...
void test_generate_probability() {
    try {
        RepositoryProbability repository_probability;
        assert(false);
    } catch (...) {
        assert(true);
    }
